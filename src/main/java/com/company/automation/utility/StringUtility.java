...
public class StringUtility {
	...
	public static String getParameterizedValue(String value) {
		...
		while (m.find()) {
			...
			value = value.replace(strToReplace, PropertyManager.getResourceBundle().getString(ReplacedString));
		}
		return value;
	}

	/**
	 * This String util method removes single or double quotes from a string if its
	 * quoted. for input string = "mystr1" output will be = mystr1 for input string
	 * = 'mystr2' output will be = mystr2
	 * 
	...
	/**
	 * Checks whether the argument <tt>string</tt> is <code>null</code> or empty.
	 * Please note that the <tt>string</tt> is <strong>trimmed</strong>, so that a
	 * check on a string containing white spaces only will always return
	 * <code>true</code>.
	 * 
	 * @param string the string to be checked
	 * @return <code>true</code> in case the argument <tt>string</tt> is
	 *         <code>null</code>, empty ({@link String#length()} returns 0) or
	 *         contains only white spaces ( <tt>{@link String#trim()}.length()</tt>
	 *         returns 0)
	 */
	...
	/**
	 * This String util method generates a random string exactly of the same size
	 * passed.
	 * 
	 * @param integer the length of the string to be generated in number.
	 * 
